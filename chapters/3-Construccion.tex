...
A continuación se presenta cómo se abordó cada historia desde el punto de vista del diseño para llevarla a su implementación descrita en el capítulo siguiente. Además al final de esta sección se entrega una visión de la arquitectura final del sistema.
...
De esta manera y dado que la técnica de etiquetado que se utilizó no es del todo precisa permitirá al usuario final discriminar si se ha llegado a un resultado es del todo correcto.
...
Se solicitó que la interfaz no se recargue cada vez que se produzca un cambio dado por un nuevo evento detectado o el modificación en el intervalo de visualización. Para ello se utilizaron las facultados de Javascript y AJAX capturando los cambios en la línea temporal, descrita en la sección \ref{subsec:HU-v04} y detallada en su implementación en \ref{subsec:HU-v04Implementacion}, y actualizando a la nueva información agregada.
...
Se consideró apropiado, además, diseñar un icono que representara la densidad de marcadores al momento de realizar el agrupamiento por categorías descrito en la sección \ref{subsec:HU-v02} para ello y siguiendo la combinación de colores descrita en la sección \ref{subsec:HU-v02Implementacion} se prepararon tres iconos adicionales a cada categoría como pueden verse en las figuras \ref{fig:aguaS}. a la \ref{fig:seguridadL}.\\
...
Estos iconos ocuparán el lugar del marcador por defecto de Google Maps y de los iconos correspondientes a los \textit{clusters} de marcadores. Para mas detalle refiérase a la sección \ref{subsec:HU-v06Implementacion}.
...
La implementación de estos algoritmos se detalla en la sección \ref{subsec:HU-c07Implementacion} correspondiente a la implementación de esta historia de usuario en el sistema.

\subsection{Historia de usuario HU-v08}
\label{subsec:HU-v08}

\textbf{TO BE DETAILED}
...
Este operador se encargará de conectarse a la base de datos y almacenar el nuevo marcador. Para ello hará uso de \textit{Jongo} para realizar la transformación, desde objeto Java a documento BSON para almacenar en MongoDB.
